...
report_text <- fs::path("files", "program") |>
  fs::dir_ls(
    glob = "*.pdf"
  ) |>
  map(
...
project_details <- tidy_report_text |>
  mutate(
    cip_num = if_else(
      key == "project",
...
project_information <- project_details |>
  fill(key) |>
  filter(
    key == "description"
  ) |>
...
project_funding <- project_details |>
  filter(
